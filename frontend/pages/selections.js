            ...
            { margin:5, cols:[
                {
                     view:"button", value:"Подтвердить" , css:"webix_primary",  click:function(id,event){
                            ...
                            webix.ajax().post("http://localhost:3000/formvalues2",formvalues2).then(function()
                            {
                                   $$('window').close()
                                   $$('datatable').load("http://localhost:3000/hello2")        
                           });
    ...
    let page_body={
        ...
        rows:[      {
            ...
            onClick:{"custom_trash":function(e,id,node){            
                ...
                .then(function(){
                    webix.ajax().post("http://localhost:3000/deleteform2",id).then(function()
                             {
                                    $$('datatable').clearAll()
                                    $$('datatable').load("http://localhost:3000/hello2")            
                            });        
